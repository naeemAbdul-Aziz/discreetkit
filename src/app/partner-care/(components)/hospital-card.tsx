...
import { Award, Bot, MessageCircle, Phone, Globe } from 'lucide-react';
...
const withUtmParams = (url: string) => {
  ...
  try {
    ...
    // Handle cases where the URL might be invalid (though it should be validated on insertion)
    return '#';
  }
};
...
export function HospitalCard({ hospital }: { hospital: Hospital }) {
  ...
  return (
    <Card className={cn(
      "flex h-full flex-col overflow-hidden rounded-2xl border bg-card p-4 transition-all duration-300",
      is_preferred ? "border-primary/50 shadow-lg" : "hover:shadow-md"
    )}>
      {is_preferred && (
      ...
      <div className="flex flex-grow flex-col pt-4">
       ...
       <div className="mt-4 text-center text-xs text-muted-foreground">
        Mention **DiscreetKit** for priority, confidential service.
      </div>
    </Card>
  );
}
