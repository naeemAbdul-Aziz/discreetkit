...
-- Generated: 2025-11-24 (Updated with pharmacy-user linking)
...
COMMENT ON TABLE public.products IS 'Stores all available products, pricing, and inventory.';

-- 3.3 PHARMACIES (UPDATED WITH USER LINKING)
...
COMMENT ON COLUMN public.pharmacies.user_id IS 'Links pharmacy record to auth user for pharmacy portal access';

-- Index and constraint for user_id
CREATE INDEX pharmacies_user_id_idx ON public.pharmacies(user_id);
CREATE UNIQUE INDEX pharmacies_user_id_unique ON public.pharmacies(user_id);
...
COMMENT ON TABLE public.orders IS 'Customer orders with delivery and pharmacy assignment details.';
...
-- Allow admin/service_role to insert, update, and delete pharmacies
